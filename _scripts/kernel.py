...
from .utils import _check_output, _run, download_file, COLOR as C
...
SYSML2_JUPYTER_INSTALLER = SYSML2_RELEASE_FOLDER / "install/jupyter/install.py"
...
PARSER.add_argument(
    ...
    type=str,
    help=f"IP address to publish SysML models to "
    "(default='{V.REMOTE_API_SERVER_URL}')",
)
...
def _get_sysml_release(force: bool = False, skip_clean: bool = False) -> list:
    ...
    try:
        ...
        if not skip_clean:
            SYSML2_JUPYTER_INSTALLER.unlink()
...
def install(
    ...
    if force_download or not SYSML2_JUPYTER_INSTALLER.exists():
    ...
    kernel_install_commands = [
        "python",
        SYSML2_JUPYTER_INSTALLER.resolve(),
        "--sys-prefix",
